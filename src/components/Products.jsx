import { MapPin, Leaf, Sparkles, CheckCircle, FlaskConical, Award, Thermometer, Flame, Palette, Star } from 'lucide-react';
...
const products = [
  ...
  },
  {
    ...
    varieties: [
      { name: 'Bhivapuri', icon: Award,       label: "India's Favourite", desc: "India's most popular variety — bold colour, strong aroma, and intense heat. Widely used in masalas and food processing." },
      { name: 'Chapata',   icon: Thermometer, label: 'Less Spicy',        desc: 'Specially known for its exceptionally bright red colour with mild heat — perfect for colour extraction, paprika, and food colouring industries.' },
      { name: 'C5',        icon: Flame,       label: 'Medium Heat',       desc: 'Medium spicy with a deep dark red colour. Preferred by exporters and spice manufacturers for consistent quality and appearance.' },
    ],
  ...
  },
  {
    ...
    varieties: [
      { name: 'Shallam (Sellem)', icon: Palette, label: 'High Colour',  desc: 'Known for its rich golden-orange colour with high colour value. Preferred for colour extraction, cosmetics, and food colouring — less emphasis on medicinal benefits.' },
      { name: 'Waigaon',         icon: Star,    label: 'Pure & Potent', desc: 'Pure turmeric with complete medicinal benefits and high curcumin content. Well-known in the Indian market for its authentic quality and strong healing properties.' },
    ],
          ...
          <div key={title} className="product-detail-card">
            ...
            <div className="pdc-content">
              ...
              {varieties && (
                <div className="pdc-varieties">
                  ...
                  <div className="pdc-variety-list">
                    {varieties.map(({ name, icon: BadgeIcon, label, desc: vDesc }) => (
                      <div key={name} className="pdc-variety">
                        <div className="pdc-variety-header">
                          <strong>{name}</strong>
                          <span className="pdc-variety-badge">
                            <BadgeIcon size={12} />
                            {label}
                          </span>
                        </div>
              ...
              <div className="pdc-trust">
                {trust.map(t => (
                  <span key={t} className="pdc-trust-badge">
